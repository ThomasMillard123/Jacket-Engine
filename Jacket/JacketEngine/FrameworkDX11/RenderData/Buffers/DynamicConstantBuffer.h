...
namespace  Engine::Core::Graphics::Buffers {
...
#define X(el) static_assert(Map<el>::valid,"Missing map implmentation for" #el);
    LEAF_ELEMENT_TYPES
    #undef X
    ...
    public:
        ...
        //creat buffer
        void CreatBuffer(ID3D11Device* dev) {
            D3D11_BUFFER_DESC bd = {};
            bd.Usage = D3D11_USAGE_DEFAULT;
            bd.ByteWidth = GetSizeInBytes();
            bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
            bd.CPUAccessFlags = 0;
            dev->CreateBuffer(&bd, nullptr, &buffer);
        }
        //update data in buffer
        void UpdateBuffer(ID3D11DeviceContext* con) {
            con->UpdateSubresource(buffer, 0, nullptr,GetData(), 0, 0);
        }
        //get buffer
